...
// פונקציית אתחול הלוח והצוללות (מתורגמת מה-VB.NET)
...
export default function BattleshipGame({ onBackToMenu }) {
  ...
  const handleCellClick = (r, c) => {
    ...
    if (board[r][c].isRevealed) return; // לא ללחוץ על מה שכבר נחשף
    setPreviewCell({ r, c });
  };
        ...
        {ships.map(ship => (
          <div key={ship.id} className="flex gap-1">
            {Array.from({ length: ship.size }).map((_, idx) => {
              let bgColor = "bg-slate-300"; // רגיל
              if (ship.isSunk) bgColor = "bg-red-800"; // הוטבעה
              else if (idx < ship.hits) bgColor = "bg-orange-500"; // פגיעה
              return (
  ...
  return (
    <div dir="rtl" className="h-screen w-full bg-[#001F3F] text-white font-sans flex flex-col md:flex-row overflow-hidden relative">
      ...
      {/* רקע ים עמוק במקום הילות פשוטות */}
      ...
      <div className="w-full md:w-[420px] lg:w-[480px] shrink-0 bg-slate-900/80 backdrop-blur-2xl border-l border-white/5 p-6 flex flex-col z-20 shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-y-auto">
        ...
        <div className="flex-1 min-h-[350px] shrink-0 bg-black/20 rounded-3xl border border-white/5 p-5 relative overflow-hidden flex flex-col">
          ...
          {gameState === 'WAITING_START' && (
            <div className="h-full flex flex-col items-center justify-center gap-6 animate-in fade-in duration-700">
              ...
              <button 
                onClick={startGame}
                className="mt-4 w-full py-4 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 font-black text-xl flex items-center justify-center gap-3 hover:scale-105 transition-all shadow-[0_0_20px_rgba(6,182,212,0.4)]"
              >
      ...
      <div className="flex-1 min-w-0 h-full flex items-center justify-center p-4 md:p-12 relative z-10">
        ...
        {/* במקום SVG, במלחמה ימית נשתמש ב-CSS Grid אמיתי בשביל הריבועים */}
        <div className="w-full h-full max-w-3xl max-h-[85vh] drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)] border-4 border-cyan-700/50 bg-[#0A1E50] rounded-xl overflow-hidden p-2"
          ...
          {board.flatMap((row, r) => 
            row.map((cell, c) => {
              ...
              let bgColor = "bg-[#05143A]"; // מים עמוקים רגיל
              ...
              if (cell.isRevealed) {
                ...
                borderStyle = `border-4 border-[${currentGroup.color}] animate-pulse`;
              ...
              return (
                <button
                  ...
                  style={isPreview ? { borderColor: currentGroup.color } : {}}
                >
                  {content}
                </button>
              );
          ...
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md overflow-hidden">
            ...
            <div className="relative animate-[spin_10s_linear_infinite] p-1.5 rounded-[42px]" style={{ background: `linear-gradient(45deg, ${getWinner().color}, transparent, ${getWinner().color})` }}>
              <div className="bg-slate-900/90 border border-white/10 rounded-[40px] p-16 flex flex-col items-center shadow-2xl animate-in zoom-in-75 duration-700"
                ...
                <p className="text-2xl text-slate-300 mb-12 font-medium">הטביעו את כל הצי עם {scores[getWinner().id]} פגיעות ישירות!</p>
